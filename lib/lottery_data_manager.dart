...
class _LotteryNumber extends Item {
  ...
  factory _LotteryNumber.fromJson(String str) => _LotteryNumber.fromMap(json.decode(str));

  factory _LotteryNumber.fromMap(Map<String, dynamic> json) => _LotteryNumber(json['number']);
}

class _LotteryNumbers extends Items {
  List<_LotteryNumber> lotteryNumbers;
  _LotteryNumbers(this.lotteryNumbers);
  factory _LotteryNumbers.fromMap(Map<String, dynamic> json) => _LotteryNumbers(json['items']) == null ? []
      : List<_LotteryNumber>.from(json['items'].map((x) => _LotteryNumber.fromMap(x)));
}
...
class LotteryDataManager {
  static final String pickUrl = "htps://www.random.org/quick-pick/?lottery=6x33.1x16&tickers=";
